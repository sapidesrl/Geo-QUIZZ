...
- **Textes d'interface** : gérés par **react-i18next** (`src/i18n/index.ts`). Les
  chaînes vivent dans `src/i18n/locales/<locale>.json` (énoncés de questions inclus,
  avec interpolation). Les libellés/descriptions de modes et de succès restent en
  français dans le code (source de vérité) et servent de `defaultValue`.
- **Noms de données** (pays, monnaies, langues, continents) : résolus dans la langue
  courante via **`Intl.DisplayNames`** (données CLDR du navigateur, hors-ligne, toutes
  locales) — voir `src/i18n/display.ts`. `Country` ne stocke que des **codes stables**
...
src/
  ...
  i18n/        index.ts (init react-i18next), locales/*.json, display.ts (Intl.DisplayNames), fr.ts (repli)
```
